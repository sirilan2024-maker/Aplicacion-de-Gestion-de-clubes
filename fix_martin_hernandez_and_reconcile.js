...
async function fixMartinHernandezAndReconcile() {
  console.log("=== FIX DEFINITIVO DE MATCHING DE MARTIN HERNANDEZ RUIZ VS EDINSON YAMPIER ===");
  ...
  const matchOurPlayerOnly = (nameRaw) => {
    ...
    for (const p of players) {
      ...
      // 1. Puntos por cada palabra del apellido (+50)
      for (const lnW of lnWords) {
        if (words.some(w => w === lnW || w.includes(lnW) || lnW.includes(w))) {
          score += 50;
        }
      }

      // 2. Puntos por cada palabra del nombre (+30)
      for (const fnW of fnWords) {
        if (words.some(w => w === fnW || w.includes(fnW) || fnW.includes(w))) {
          score += 30;
        }
      }

      // Requisito indispensable: Debe existir al menos 1 coincidencia en los apellidos
      const hasLnMatch = lnWords.some(lnW => words.some(w => w === lnW || w.includes(lnW) || lnW.includes(w)));
      if (!hasLnMatch) {
        score = 0;
      }
  ...
  console.log("✅ FIX DEFINITIVO REALIZADO CON ÉXITO.");
}

fixMartinHernandezAndReconcile();
