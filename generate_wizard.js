        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-6 mb-8">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-gray-600 mb-4">No hay equipos en la nueva temporada. Selecciona los equipos que deseas crear idénticos (solo su estructura, sin jugadores).</p>
              
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 mb-4">
                {pastTeams.map(t => (
                  <label key={t.id} className="flex items-center gap-3 p-3 bg-white border rounded-lg cursor-pointer hover:border-blue-300">
                    <input 
                      type="checkbox" 
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="p-4 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <button
            ...
            className="w-full sm:w-auto flex items-center justify-center gap-2 bg-green-600 text-white px-6 py-2.5 rounded-lg font-medium hover:bg-green-700 transition-colors disabled:opacity-50"
          >
          ...
          <table className="w-full text-left text-sm text-gray-600">
            <thead className="bg-gray-50 text-gray-700 uppercase font-semibold text-xs border-b">
              <tr>
                ...
                <th className="px-6 py-4">Equipo Asignado ({activeSeason?.name})</th>
              </tr>
            </thead>
                ...
                filteredPlayers.map(player => {
                  ...
                  return (
                    <tr key={player.id} className="hover:bg-blue-50/50 transition-colors group">
                      ...
                      <td className="px-6 py-3">
                        <select 
                          className={\w-full border rounded-lg px-3 py-1.5 outline-none transition-colors \\}
                          value={currentTeamId}
...
fs.writeFileSync('src/app/admin/temporadas/asistente/page.tsx', code);
console.log('generated wizard UI successfully');
