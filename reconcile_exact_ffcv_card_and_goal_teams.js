...
async function reconcileExactFfcvCardAndGoalTeams() {
  console.log("=== RECONCILIANDO ESTRICTAMENTE TARJETAS Y GOLES POR JUGADORES Y EQUIPOS FFCV ===");
  ...
  // Algoritmo ponderado de coincidencia exacta de jugador
  const matchPlayer = (nameRaw) => {
    ...
    for (const p of players) {
      ...
      // Coincidencia en Apellidos (Peso mayor)
      ...
      // Requisito indispensable: Debe existir coincidencia en al menos 1 palabra del apellido para evitar falsos positivos
      if (lnWords.length > 0) {
  ...
  for (const m of matches) {
    ...
    console.log(`✅ Partido vs ${m.rival_nombre}: Reconciliados ${golesLocalList.length + golesVisitanteList.length} Goles y ${tarjetasLocalList.length + tarjetasVisitanteList.length} Tarjetas.`);
  }

  console.log("\n=======================================================");
  console.log("✅ PARSER EXACTO Y MATCHING DE JUGADORES 100% FINALIZADO");
  console.log("=======================================================");
}

reconcileExactFfcvCardAndGoalTeams();
