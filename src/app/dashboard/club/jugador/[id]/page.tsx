...
import { 
  ...
  Save, AlertCircle, Camera, UploadCloud, Loader2, X, TrendingUp, AlertTriangle, FolderOpen, Shield, Trash2, ChevronRight
} from "lucide-react";
...
export default function GlobalPlayerProfilePage() {
  ...
  const fetchPlayer = async () => {
    ...
    try {
      ...
      if (data?.team_id) {
        const { data: tData } = await supabase.from('teams').select('category, name').eq('id', data.team_id).single();
        if (tData) {
