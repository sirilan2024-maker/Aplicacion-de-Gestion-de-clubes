// src/components/features/club/EstadisticasView.tsx
"use client"

import { useEffect, useState, useMemo } from "react"
import { createClient } from "@/lib/supabase/client"
import { BarChart3, Users, Activity, TrendingUp, Goal, AlertTriangle, Clock, Ruler, Scale, Filter, History, LayoutGrid, Sparkles, Trophy, Shield } from "lucide-react"
...
export function EstadisticasView({ fixedTeamId }: { fixedTeamId?: string }) {
  ...
  const [viewMode, setViewMode] = useState<'global' | 'individual' | 'rendimiento'>('global')
  const [showGlobalTrendsModal, setShowGlobalTrendsModal] = useState(false)
  const [teamMatches, setTeamMatches] = useState<any[]>([])
  const [globalStats, setGlobalStats] = useState<{ kpis: GlobalStatsKPIs; teamStats: TeamStatDTO[] } | null>(null)
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      try {
        ...
        console.error("Error al cargar estadísticas consolidadas:", err)
      }
    ...
    if (typeof window !== 'undefined') {
      ...
      if (params.get('view') === 'rendimiento') {
        setViewMode('rendimiento')
      }
    }
  ...
  const filteredTeamStats = useMemo(() => {
    if (!globalStats?.teamStats) return []
    if (selectedTeamId === "todos") return globalStats.teamStats
    return globalStats.teamStats.filter(t => t.teamId === selectedTeamId)
  }, [globalStats, selectedTeamId])
    ...
    <div className="w-full max-w-7xl mx-auto p-4 md:p-6 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-white p-4 rounded-2xl shadow-sm border border-slate-200">
        <div className="flex bg-slate-100 p-1.5 rounded-xl w-full md:w-auto">
          ...
          <button
            ...
            className={`flex-1 md:flex-none px-4 py-2 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2 ${
              viewMode === 'rendimiento' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
            <Shield size={16} />
      ...
      {viewMode === 'rendimiento' ? (
        <div className="space-y-6 animate-in fade-in duration-300">
          {/* Ventana: Efectividad Global */}
          <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-sm flex flex-col justify-between">
            ...
            <div className="mt-4 flex items-baseline gap-2">
              <span className="text-4xl sm:text-5xl font-black tracking-tight">{globalStats?.kpis.winRate || 0}%</span>
              <span className="text-sm text-blue-100 font-medium">victorias sobre partidos jugados</span>
            </div>
            <div className="mt-4 flex flex-wrap items-center gap-6 text-sm font-medium text-blue-100 pt-3 border-t border-blue-500/40">
              <span><b>{globalStats?.kpis.wins || 0}</b> Victorias</span>
              <span><b>{globalStats?.kpis.draws || 0}</b> Empates</span>
              <span><b>{globalStats?.kpis.losses || 0}</b> Derrotas</span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-4 sm:p-5 border-b border-slate-100 flex items-center justify-between">
              ...
              <span className="text-xs text-slate-500 font-bold bg-slate-100 px-2.5 py-1 rounded-full">
                {filteredTeamStats.length} equipos listados
              </span>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/80 border-b border-slate-200 text-xs font-bold text-slate-500 uppercase tracking-wider">
                    ...
                    <th className="py-3.5 px-4 text-center text-emerald-700">V</th>
                    <th className="py-3.5 px-4 text-center text-slate-600">E</th>
                    <th className="py-3.5 px-4 text-center text-rose-700">D</th>
                    <th className="py-3.5 px-4 text-center">GF</th>
                    <th className="py-3.5 px-4 text-center">GC</th>
      ...
      <div>
        ...
        {/* Ventana: Efectividad Global */}
        <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-5 sm:p-6 text-white shadow-sm flex flex-col justify-between mb-6">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-blue-100">
              Efectividad Global
            </span>
          ...
          <div className="mt-4 flex items-baseline gap-2">
            <span className="text-4xl sm:text-5xl font-black tracking-tight">{globalStats?.kpis.winRate || 0}%</span>
            <span className="text-xs sm:text-sm text-blue-100 font-medium">victorias sobre partidos jugados</span>
          </div>
          <div className="mt-4 flex flex-wrap items-center gap-4 sm:gap-6 text-xs sm:text-sm font-medium text-blue-100 pt-3 border-t border-blue-500/40">
            <span><b>{globalStats?.kpis.wins || 0}</b> Victorias</span>
            <span><b>{globalStats?.kpis.draws || 0}</b> Empates</span>
            <span><b>{globalStats?.kpis.losses || 0}</b> Derrotas</span>
          </div>
