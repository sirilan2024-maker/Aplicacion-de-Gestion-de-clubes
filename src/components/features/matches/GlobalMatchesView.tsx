...
import { Swords, Calendar, Clock, MapPin, User, Pencil, Trash2, Plus, Users, AlertCircle, Search, Filter, Trophy, CalendarCheck, ChevronRight, ClipboardCheck, FileText } from "lucide-react"
...
export function GlobalMatchesView({ initialMatches, teams, players = [], convocatorias = [], fixedTeamId, isReadOnly = false, userRole = '', userTeamIds = [] }: GlobalMatchesViewProps) {
  ...
  const initialView = (searchParams.get('view') as 'partidos' | 'clasificacion' | 'disciplina' | 'en-directo' | 'actas') || 'partidos'
            ...
            >
              <div className="p-5 flex-1 flex flex-col">
                ...
                {(() => {
                  ...
                  const homeScore = isLocal ? (match.resultado_propio ?? 0) : (match.resultado_rival ?? 0);
                  const awayScore = isLocal ? (match.resultado_rival ?? 0) : (match.resultado_propio ?? 0);
                        ...
                        {match.estado === 'Programado' ? (
                          ...
                          <div className="flex flex-col items-center">
                            <div className="flex items-center gap-2">
                              <span className={`text-2xl font-black ${homeScore > awayScore ? "text-emerald-600" : "text-slate-700"}`}>{homeScore}</span>
                              <span className="text-slate-300 font-bold">-</span>
                              <span className={`text-2xl font-black ${awayScore > homeScore ? "text-emerald-600" : "text-slate-700"}`}>{awayScore}</span>
                            </div>
                    ...
                    >
                      ...
                      <div className="flex gap-1.5">
                        <button 
                          ...
                          title="Ver / Conciliar Acta Oficial"
                        >
