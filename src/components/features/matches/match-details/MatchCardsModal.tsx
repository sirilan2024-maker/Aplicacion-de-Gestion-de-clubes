...
interface MatchCardsModalProps {
  matchId: string
  players: Player[]
  convocatorias: any[]
  onClose: () => void
...
export function MatchCardsModal({ matchId, players, convocatorias, onClose }: MatchCardsModalProps) {
  ...
  const initialStats: Record<string, { yellows: number, reds: number }> = {};
  ...
  const handleUpdate = (playerId: string, field: 'yellows' | 'reds', delta: number) => {
    setStats(prev => {
  ...
  const StatControl = ({ playerId, field, icon, max }: { playerId: string, field: 'yellows'|'reds', icon: string, max?: number }) => {
          ...
          {playerList.map(player => (
            <div key={player.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 border border-slate-150 rounded-xl bg-white hover:border-slate-300 transition-colors gap-3">
              ...
              <div className="flex items-center gap-2 overflow-x-auto pb-1 sm:pb-0 hide-scrollbar justify-start sm:justify-end flex-1">
                <StatControl playerId={player.id} field="yellows" icon="Amar" max={2} />
                <StatControl playerId={player.id} field="reds" icon="Roja" max={1} />
              </div>
            </div>
          ))}
