...
export function MatchHeader({ localGoals, awayGoals, goalsList, match, allMatches }: MatchHeaderProps) {
  const isLocal = match?.lugar === 'Local' || true; // Asumimos local si no hay datos
  const teamName = match?.equipo?.name || "Sporting Saladar";
  const rivalName = match?.rival_nombre || "Atlético Écija";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex overflow-x-auto gap-4 pb-3 pt-1 scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
        {(allMatches || []).map((m: any, index: number) => {
          const isCurrent = m.id === match?.id;
          const matchDate = new Date(m.fecha_hora);
          const isFinished = m.estado === 'Finalizado';
          const title = m.competicion_nombre || `Jornada ${index + 1}`;
          
          let score = "VS";
          if (isFinished || m.estado === 'En Curso' || m.estado === 'Descanso') {
            score = `${m.resultado_propio ?? 0} - ${m.resultado_rival ?? 0}`;
          }
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between text-[10px] font-bold text-slate-400">
                <span className={isCurrent ? "text-blue-600 truncate" : "truncate"}>{title}</span>
                <span>{isFinished ? "🏆" : (isCurrent ? "⚡" : "⚽")}</span>
              </div>
              <div className="my-2">
                <p className="text-xs font-bold text-slate-900 truncate">vs {m.rival_nombre}</p>
                <p className="text-[10px] text-slate-400 font-semibold">{matchDate.toLocaleDateString('es-ES', { day: '2-digit', month: 'short' })}</p>
              </div>
              <div className="flex justify-between items-center mt-1">
                <span className={[
                  ...
                  {isCurrent ? `${localGoals} - ${awayGoals}` : score}
                </span>
                {isFinished && (
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-150 overflow-hidden">
        ...
        <div className="bg-blue-600 text-white px-6 py-4 flex justify-between items-center text-xs font-bold tracking-wide">
          ...
          <span className="opacity-90">{matchDate.toLocaleDateString('es-ES', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</span>
        </div>
