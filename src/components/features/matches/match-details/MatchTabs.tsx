  ...
  return (
    <>
      <div className="sticky top-0 bg-white z-20 shadow-sm border-b border-slate-150 -mx-4 md:-mx-8 px-4 md:px-8 mb-4 md:mb-0">
        ...
        {/* Mobile Dropdown Destacado */}
        <div className="md:hidden py-2 px-1">
          <div className="relative">
            <select
              value={activeTab}
              onChange={(e) => onChangeTab(e.target.value as TabType)}
              className="w-full bg-white border-2 border-blue-600 text-slate-800 text-xs font-extrabold rounded-xl px-4 py-3 outline-none appearance-none shadow-sm focus:ring-2 focus:ring-blue-500 pr-10"
            >
              {TAB_OPTIONS.map((tab) => (
                <option key={tab.id} value={tab.id} className="bg-white text-slate-800 font-bold py-1">
