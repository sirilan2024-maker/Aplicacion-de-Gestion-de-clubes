...
import { Save, UploadCloud, CheckCircle, Image as ImageIcon, X, Plus, Trash2, GraduationCap, Briefcase, Home, FileText, UserCircle, ChevronDown, ChevronUp } from "lucide-react";
...
export function Step2Documents() {
  ...
  const isSeniorTeam = useWatch({ control, name: "isSeniorTeam" });
  const isSeniorSelection = useWatch({ control, name: "isSeniorSelection" });
  const birthDate = useWatch({ control, name: "birthDate" });
  const uploadedFiles = useWatch({ control, name: "uploadedFiles" }) || [];
  
  const isSenior = isSeniorTeam === true || isSeniorTeam === "true" || isSeniorSelection === "senior" || (birthDate ? new Date(birthDate).getFullYear() <= 2007 : false);
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {!isSenior && (
        <div className="bg-orange-50/30 rounded-2xl border border-orange-200 shadow-sm overflow-hidden transition-all duration-300">
          <div 
            onClick={() => setIsExpedienteOpen(!isExpedienteOpen)}
            className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 cursor-pointer hover:bg-orange-100/40 transition-colors select-none"
          >
            <div className="flex items-start sm:items-center gap-3">
              <span className="flex items-center justify-center w-7 h-7 rounded-full bg-blue-700 text-white text-xs font-bold shrink-0 mt-0.5 sm:mt-0">2</span>
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <h4 className="font-bold text-blue-950 text-base">Expediente Especial Jugadores Extranjeros (FFCV / FIFA)</h4>
                  <span className="text-[11px] text-orange-800 bg-orange-100 font-bold px-2.5 py-0.5 rounded-full border border-orange-200">
                    FIFA Art. 19
                  </span>
                </div>
                <p className="text-xs text-blue-900/80 font-medium mt-0.5">
                  Pulsa aquí para {isExpedienteOpen ? 'plegar' : 'desplegar'} si el jugador es extranjero y necesitas adjuntar la documentación federativa.
                </p>
              </div>
            </div>

            <div className="flex items-center gap-2 self-end sm:self-center shrink-0">
              <span className="text-xs font-bold text-blue-700 bg-white border border-blue-200 py-1.5 px-3 rounded-lg flex items-center gap-1.5 shadow-xs">
                {isExpedienteOpen ? (
                  <>Plegar <ChevronUp className="w-4 h-4 text-blue-600" /></>
                ) : (
                  <>Desplegar <ChevronDown className="w-4 h-4 text-blue-600" /></>
                )}
              </span>
            </div>
          </div>
          ...
          {isExpedienteOpen && (
            <div className="p-6 pt-2 space-y-6 border-t border-orange-200/80 animate-in fade-in duration-300">
              {/* Banner Informativo Normativa FIFA Art. 19 */}
              <div className="bg-white p-5 rounded-xl border border-orange-200 space-y-3 mt-3">
