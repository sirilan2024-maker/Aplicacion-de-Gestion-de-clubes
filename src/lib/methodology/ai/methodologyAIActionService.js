/**
 * Servicio de Acciones y Copiloto Operativo Metodológico v1.0 (JS para tests y runtime)
 * Antigravity Methodology OS - Fase 5.2
 * 
 * Principio Arquitectónico:
 * IA propone -> Humano revisa -> Motor Determinista valida -> Humano confirma -> Sistema persiste
 * 0 escrituras directas en Base de Datos por la IA.
 */

const { 
  regenerateSessionBlock, 
  validateMethodologySessionProposal 
} = require("../methodologySessionGenerator");
const { 
  regenerateMicrocycleDay, 
  validateMicrocycleProposal 
} = require("../methodologyMicrocyclePlanner");

function buildSessionActionProposal(params) {
  const { 
...
function buildMicrocycleActionProposal(params) {
  const {
...
function validateAIActionProposal(proposal, currentObject, context) {
  const errors = [];
...
function applyAIActionToLocalState(proposal, currentObject, context) {
  ...
  if (proposal.type === 'regenerate_session_block') {
    ...
    const updatedProposal = regenerateSessionBlock(currentObject, blockId, context);
  ...
  if (proposal.type === 'regenerate_microcycle_day') {
    ...
    const updatedMicro = regenerateMicrocycleDay(currentObject, dayOfWeek, context);
...
function getActionImpactPreview(proposal, currentObject, context) {
  ...
  if (isSession) {
    const beforeMetrics = {
      ...
      exerciseCount: Object.values(currentObject?.blocks || {}).reduce((sum, b) => sum + (b.exercises?.length || 0), 0)
    };
    ...
    const afterMetrics = {
      ...
      exerciseCount: Object.values(updatedObject?.blocks || {}).reduce((sum, b) => sum + (b.exercises?.length || 0), 0)
    };
    ...
    const targetDayBefore = currentObject?.days?.find(d => d.dayOfWeek === dayOfWeek);
    ...
    const targetDayAfter = updatedObject?.days?.find(d => d.dayOfWeek === dayOfWeek);
