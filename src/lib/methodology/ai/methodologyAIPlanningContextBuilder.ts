/**
 * Constructor de Contexto de Planificación Metodológica IA v1.0 (TypeScript)
...
export interface TeamPlanningContextParams {
  club: { id: string; name: string };
  team: { id: string; name: string; category: string };
  season: { id: string; name: string };
  report?: Partial<SeasonMethodologyReport> | any;
  priorities?: any[];
...
export function buildTeamPlanningAIContext(params: TeamPlanningContextParams): MethodologyAIContext {
  const {
    club,
    team,
    season,
