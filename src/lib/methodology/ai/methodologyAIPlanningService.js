/**
 * Servicio de Planificación Metodológica Asistida por IA v1.0 (JS & TS)
...
function buildAIPlanningProposal(context, curriculumPrinciples = [], allExercises = []) {
  const {
    team,
    season,
    planningContext = {},
  ...
  // 1. Delegar en el motor determinista de microciclos
  const deterministicMicro = generateMicrocycleProposal({
    teamId: team.id,
  ...
  // 2. Mapear días con campos normalizados
  const proposedDays = deterministicMicro.days.map(d => {
  ...
  // 3. Proponer plantillas de sesión para los días de entrenamiento
  const proposedSessions = deterministicMicro.days
  ...
  return {
    ...
    currentMethodologyState: {
      status: isInsufficient ? 'datos_insuficientes' : (teamReport.summary?.avgObjectiveAchievement < 2.2 ? 'atencion' : 'solido'),
...
function updateAIPlanningDay(proposal, dayOfWeek, dayEdits, curriculumPrinciples = []) {
  if (!proposal || !proposal.proposedMicrocycle || !Array.isArray(proposal.proposedMicrocycle.days)) {
    return { proposal, valid: false, errors: ['Propuesta de planificación inválida'] };
  }
