/**
 * Servicio de Feedback Metodológico Post-Sesión con IA v1.0 (TypeScript)
 * Antigravity Methodology OS - Fase 5.4
 */

import { calculatePlannedVsExecutedFeedback, PlannedVsExecutedFeedbackResult } from "../sessionExecutionFeedbackService";
import { buildSessionActionProposal, buildMicrocycleActionProposal } from "./methodologyAIActionService";
import { MethodologyAIActionProposal } from "./types";

export interface PostSessionFeedbackResult {
  answer: string;
  facts: string[];
  ...
  comparison: PlannedVsExecutedFeedbackResult;
  actionProposals: MethodologyAIActionProposal[];
}

export function generatePostSessionFeedback(params: {
  session: any;
  evaluation?: any;
  attendance?: any[];
  history?: any[];
}): PostSessionFeedbackResult {
