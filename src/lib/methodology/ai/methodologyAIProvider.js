/**
 * Proveedor de Inteligencia Artificial Metodológica v1.0 (JS para tests y runtime)
...
const { buildAIPlanningProposal } = require("./methodologyAIPlanningService");

class MethodologyAIProvider {
  constructor(apiKey) {
...
${prompt}`;
  ...
  generateDeterministicAnalysis(prompt, context) {
    ...
    if (isClub) {
      ...
      const facts = [
        ...
        `Consecución media: ${summary.avgObjectiveAchievement?.toFixed(1) || 0}/4.`,
      ...
      return {
        ...
        evidence: [
          ...
          { metric: "Consecución", value: `${summary.avgObjectiveAchievement?.toFixed(1) || 0}/4`, reference: context.team?.name || "Equipo" }
        ],
        dataSufficiency: {
          sufficient: !isInsufficient,
          sampleSize,
