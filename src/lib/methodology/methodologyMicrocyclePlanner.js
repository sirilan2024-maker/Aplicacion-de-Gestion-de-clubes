/**
 * Planificador Metodológico de Microciclos Determinista v1.0 (Versión JS para tests y runtime)
 * Antigravity Methodology OS
 */

const DAY_NAMES = ["", "Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado", "Domingo"];

function calculateMdCode(dayDateStr, matchDateStr) {
...
function generateMicrocycleProposal(context) {
  const {
    teamId,
    category,
    seasonId,
  ...
  const defaultTrainingDays = context.trainingDays || [2, 4, 5];
  const baseDate = new Date(weekStartDate);

  const days = [];
  const coveredPrinciplesSet = new Set();
...
function regenerateMicrocycleDay(proposal, dayOfWeek, context) {
...
function validateMicrocycleProposal(proposal) {
  const errors = [];
  const warnings = [];
...
function convertMicrocycleDayToSessionContext(day, team, allExercises, recentExerciseIds = []) {
  return {
    teamId: team.id,
