/**
 * Motor de Decisión y Priorización Metodológica Determinista v1.0 (Versión JS para tests y runtime)
...
function calculateMethodologyPriorities(context) {
  const priorities = [];
  const currentDate = context.date ? new Date(context.date) : new Date();

  const sessions = context.recentSessions || context.history || context.historySummary?.recentSessions || [];
  const summary = context.summary || context.historySummary;
  const curriculumPrinciples = context.curriculumPrinciples || [];

  // 1. ANÁLISIS DE PRINCIPIOS
  const principleLastDate = {};
  ...
  if (curriculumPrinciples.length > 0) {
    ...
    curriculumPrinciples.forEach(cp => {
      if (!workedPrinciplesLower.has(cp.name.toLowerCase())) {
        const pLvl = sessions.length >= 3 ? 'high' : 'medium';
        priorities.push({
  ...
  // B) Principios OBSOLETOS (>= 21 días)
  Object.entries(principleLastDate).forEach(([pName, data]) => {
    ...
    // C) Principios con BAJA CONSECUCIÓN (<= 2.2)
  ...
  // 2. COMPORTAMIENTOS OBSERVABLES (N >= 3)
  ...
  behaviourEvolution.forEach(b => {
    if (b.sampleSize >= METHODOLOGY_RULES.MIN_TREND_OBSERVATIONS) {
      if (b.trend === 'declining' || b.avgScore <= METHODOLOGY_RULES.LOW_ACHIEVEMENT_THRESHOLD) {
        const pLvl = b.avgScore <= 2.0 ? 'high' : 'medium';
        priorities.push({
          ...
          suggestedPrinciple: b.gamePhaseOrFamily,
  ...
  return priorities.sort((a, b) => {
    ...
    const aScore = a.evidence.avgScore !== undefined ? a.evidence.avgScore : (a.evidence.daysSinceLastWork ? -a.evidence.daysSinceLastWork : 0);
    const bScore = b.evidence.avgScore !== undefined ? b.evidence.avgScore : (b.evidence.daysSinceLastWork ? -b.evidence.daysSinceLastWork : 0);
