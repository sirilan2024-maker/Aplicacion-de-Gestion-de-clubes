/**
 * Servicio de Memoria Metodológica de Temporada y Dirección Deportiva v1.0
 * Antigravity Methodology OS
 * Análisis longitudinal determinista: Planificación -> Ejecución -> Evaluación -> Memoria de Temporada
 */

import { 
  getTeamMethodologyHistory, 
  getBehaviourEvolution, 
  getPrincipleCoverage, 
  getSessionLoadEvolution,
  BehaviourEvolutionRecord,
  PrincipleCoverageSummary,
  SessionLoadEvolutionRecord,
  AnalyticsFilters
...
export interface SeasonMethodologySummary {
  ...
  durationDeviationMin: number;
  avgRpe: number;
  avgObjectiveAchievement: number;
  avgAttendanceRate: number;
  modelCoveragePercentage: number;
  trainedPrinciplesCount: number;
  neverTrainedPrinciplesCount: number;
  lowAchievementPrinciplesCount: number;
  improvingBehavioursCount: number;
  stableBehavioursCount: number;
  decliningBehavioursCount: number;
  insufficientDataBehavioursCount: number;
}
...
export interface SeasonObjectiveProgress {
  ...
  status: 'achieved' | 'progressing' | 'at_risk' | 'insufficient_data';
  statusReason: string;
}

export interface SeasonPrincipleDetail {
  principleId?: string;
  principleName: string;
  gamePhase: string;
  sessionsCount: number;
  classification: 'muy_trabajado' | 'trabajado' | 'poco_trabajado' | 'nunca_trabajado';
...
export interface SeasonPrioritiesSummary {
  resolvedPriorities: { title: string; resolutionEvidence: string }[];
  recurrentPriorities: { title: string; occurrences: number; explanation: string }[];
  openPriorities: { title: string; reason: string }[];
}

export interface SeasonConclusion {
  type: 'strength' | 'gap' | 'improvement' | 'risk';
  title: string;
  evidence: string;
...
export interface DataQualityReport {
  ...
  unevaluatedSessionsCount: number;
  unevaluatedSessionsPercentage: number;
  insufficientDataBehavioursCount: number;
  unassessedObjectivesCount: number;
  notes: string[];
}

export interface SeasonMethodologyReport {
  team: { id: string; name: string; category?: string };
  season: { id: string; name: string; startDate?: string; endDate?: string };
  summary: SeasonMethodologySummary;
  objectivesProgress: SeasonObjectiveProgress[];
  principlesCoverage: SeasonPrincipleDetail[];
  behaviourEvolution: BehaviourEvolutionRecord[];
  loadEvolution: SessionLoadEvolutionRecord[];
  prioritiesAnalysis: SeasonPrioritiesSummary;
  conclusions: SeasonConclusion[];
...
/**
 * Calcula la memoria metodológica completa de una temporada para un equipo
 */
export function buildSeasonMethodologyReportFromData(params: {
  team: { id: string; name: string; category?: string };
  season: { id: string; name: string; startDate?: string; endDate?: string };
  sessions: any[];
  ...
  const principleStats: Record<string, { count: number; scores: number[]; lastDate?: string; gamePhase: string }> = {};
  ...
  const principlesCoverage: SeasonPrincipleDetail[] = Object.entries(principleStats).map(([pName, data]) => {
    let classification: 'muy_trabajado' | 'trabajado' | 'poco_trabajado' | 'nunca_trabajado' = 'nunca_trabajado';
  ...
  const behaviourMap: Record<string, { scores: { date: string; sessionId: string; score: number }[]; family?: string }> = {};

  sessions.forEach(s => {
    const sDate = s.date_time || '';
    const behavioursEval = s.session_evaluations?.[0]?.session_behaviour_evaluations || [];
    behavioursEval.forEach((b: any) => {
  ...
  const behaviourEvolution: BehaviourEvolutionRecord[] = Object.entries(behaviourMap).map(([desc, bData]) => {
  ...
  const loadEvolution: SessionLoadEvolutionRecord[] = sessions.map(s => {
    const evalData = s.session_evaluations?.[0];
  ...
  const objectivesProgress: SeasonObjectiveProgress[] = teamObjectives.map(obj => {
    const descLower = (obj.description || '').toLowerCase();
    const relatedSessions = sessions.filter(s => {
      const sObj = (s.objective || '').toLowerCase();
      const sSec = (s.objectives_secondary || []).map((x: string) => x.toLowerCase());
      return sObj.includes(descLower) || descLower.includes(sObj) || sSec.some((x: string) => x.includes(descLower) || descLower.includes(x));
    });
    ...
    let status: 'achieved' | 'progressing' | 'at_risk' | 'insufficient_data' = 'insufficient_data';
  ...
  const resolvedPriorities: { title: string; resolutionEvidence: string }[] = [];
  const recurrentPriorities: { title: string; occurrences: number; explanation: string }[] = [];
  const openPriorities: { title: string; reason: string }[] = [];

  // Regla determinista de resolución: si un comportamiento con N >= 3 pasó a tener avgScore >= 3.0
  ...
  const conclusions: SeasonConclusion[] = [];
  ...
  const summary: SeasonMethodologySummary = {
...
/**
 * Comparativa metodológica multi-equipo para la Dirección Deportiva
 */
export function getTeamsMethodologyComparisonFromData(
  teamsReports: SeasonMethodologyReport[]
): TeamComparisonItem[] {
