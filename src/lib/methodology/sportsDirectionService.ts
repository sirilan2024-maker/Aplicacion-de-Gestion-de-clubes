/**
 * Servicio Central de Dirección Deportiva y Análisis Transversal v1.0
 * Antigravity Methodology OS - Fase 4.9
 * Visión global del club, comparación de equipos, alertas transversales y control metodológico
 */

import { 
  SeasonMethodologyReport, 
  SeasonMethodologySummary,
  SeasonPrincipleDetail,
  BehaviourEvolutionRecord 
} from "./seasonMethodologyReportService";
import { METHODOLOGY_RULES } from "./methodologyPriorityEngine";

export type MethodologyHealthStatus = 'solido' | 'en_seguimiento' | 'atencion' | 'datos_insuficientes';

export interface TeamMethodologyStatusDetail {
  status: MethodologyHealthStatus;
  statusLabel: string;
...
export interface TransversalAlert {
  id: string;
  teamId: string;
  teamName: string;
  category: string;
  ruleActivated: string;
  type: 'principle_gap' | 'stale_principle' | 'declining_behaviour' | 'low_achievement' | 'high_rpe_persistent' | 'low_evaluation_rate' | 'insufficient_data';
  title: string;
  description: string;
  evidence: {
    sampleSize?: number;
...
export interface ClubGlobalKpis {
  activeTeamsCount: number;
  totalPlannedSessions: number;
  totalCompletedSessions: number;
  totalEvaluatedSessions: number;
  globalEvaluationPercentage: number;
  globalAvgRpe: number;
  globalAvgAchievement: number;
  globalAvgAttendance: number;
  globalModelCoverage: number;
  teamsSolidCount: number;
  teamsMonitoringCount: number;
  teamsAttentionCount: number;
  teamsInsufficientDataCount: number;
}

export interface TeamRowOverview {
  teamId: string;
  teamName: string;
  category: string;
  ...
  modelCoveragePercentage: number;
  statusDetail: TeamMethodologyStatusDetail;
  activePrioritiesCount: number;
  decliningBehavioursCount: number;
  dataQualityNotes: string[];
}

export interface ClubMonthlyEvolutionPoint {
...
/**
 * Evalúa de manera matemática y 100% determinista el estado metodológico de un equipo
 */
export function evaluateTeamMethodologyStatus(summary: SeasonMethodologySummary): TeamMethodologyStatusDetail {
  const metrics = {
...
/**
 * Calcula los KPIs globales agregados del club a partir de las memorias de los equipos
 */
export function calculateClubGlobalKpis(reports: SeasonMethodologyReport[]): ClubGlobalKpis {
...
/**
 * Genera la matriz de equipos con estado y métricas transversales
 */
export function buildClubTeamsMatrix(reports: SeasonMethodologyReport[]): TeamRowOverview[] {
...
/**
 * Genera la lista de alertas metodológicas transversales para la Dirección Deportiva
 */
export function generateClubTransversalAlerts(reports: SeasonMethodologyReport[]): TransversalAlert[] {
  const alerts: TransversalAlert[] = [];
  ...
  reports.forEach(r => {
    ...
    // A) Alerta de Cobertura de Currículo Insuficiente
    ...
    // B) Alerta de Comportamientos en Declive Confirmado (N >= 3)
    ...
    // C) Alerta de Baja Consecución en Principios Clave
    ...
    // D) Alerta de Falta de Evaluaciones Post-Sesión
  ...
  // Ordenamiento determinista de alertas: Mayor severidad primero, luego por nombre de equipo
...
/**
 * Compara detalladamente dos o más equipos seleccionados
 */
export function compareSpecificTeams(reports: SeasonMethodologyReport[], selectedTeamIds: string[]) {
...
/**
 * Agrupa cronológicamente las sesiones de todos los equipos mes a mes
 */
export function calculateClubMonthlyEvolution(allSessions: any[]): ClubMonthlyEvolutionPoint[] {
  const monthMap: { [key: string]: any[] } = {};

  allSessions.forEach(s => {
    if (!s.date_time) return;
    const monthKey = s.date_time.substring(0, 7); // YYYY-MM
  ...
  return monthKeys.map(key => {
    ...
    return {
      ...
      avgAchievement,
      avgRpe,
      avgAttendance: 90, // Default coherente si no hay lista de ausentes
